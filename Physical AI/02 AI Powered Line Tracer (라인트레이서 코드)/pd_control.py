# pd_control.py
# PAI-Car v1.0 PD 제어 라인트레이싱 + 주행 시간 측정
#
...
#
#   P 제어는 현재 차가 라인 중심에서 얼마나 벗어나 있는지를 보고
#   방향을 수정한다.
#
...
# KD:
#   D 제어에서 error 변화량에 곱해지는 미분 제어 계수이다.
#
#   D 제어는 error가 얼마나 빠르게 변하고 있는지를 보고
#   방향 수정을 보완한다.
#
#   KD를 적절히 사용하면 좌우 흔들림을 줄이고,
#   코너 진입/탈출 시 움직임을 더 부드럽게 만들 수 있다.
#
#   KD가 너무 작으면 D 제어 효과가 거의 없다.
#   KD가 너무 크면 작은 센서 변화에도 민감하게 반응하여
#   오히려 주행이 불안정해질 수 있다.
KD = 0.17
...
#   3. 현재 error와 이전 error의 차이로 d_error를 계산한다.
#   4. KP와 KD를 이용해 correction 값을 계산한다.
#   5. correction 값으로 왼쪽/오른쪽 모터 속도를 다르게 만든다.
#   6. 계산된 속도로 모터를 구동한다.
#   7. OLED에 경과 시간을 표시한다.
#   8. 일정한 제어 주기가 되도록 기다린다.

finished = False

# D 제어를 위해 이전 error 값을 저장해 둔다.
# 첫 반복에서는 이전 error가 없으므로 0에서 시작한다.
last_error = 0
...
try:
    while True:
        ...
        # 라인을 정상적으로 감지한 경우에만 PD 제어를 수행한다.
        if on_line:

            # D 제어를 위한 error 변화량을 계산한다.
            #
            # d_error = 현재 error - 이전 error
            #
            # d_error가 크다는 것은 error가 빠르게 변하고 있다는 뜻이다.
            # 즉, 차가 라인 중심에서 빠르게 멀어지거나,
            # 반대로 빠르게 중심 쪽으로 돌아오고 있다는 의미이다.
            ...
            #
            # KP × error:
            #   현재 라인 중심에서 벗어난 정도에 대한 보정
            #
            # KD × d_error:
            #   error가 변하는 속도에 대한 보정
            #
            # P 제어만 사용할 때보다 D 항을 추가하면
            # 좌우 흔들림을 줄이고, 급격한 변화에 더 빠르게 반응할 수 있다.
            correction = int(KP * error + KD * d_error)


            # 다음 반복에서 D 제어를 계산할 수 있도록
            # 현재 error를 last_error에 저장한다.
            last_error = error
        ...
        # 라인을 정상적으로 감지하지 못한 경우
            ...
            # 라인을 다시 찾은 직후 D 값이 갑자기 커지는 것을 줄이기 위해
            # last_error를 0으로 초기화한다.
            last_error = 0
